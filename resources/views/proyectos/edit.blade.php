<x-layouts.layout>
    ...
    <div class="max-w-lg mx-auto my-10 p-6 rounded-lg shadow-lg">
        <form onsubmit=event.preventDefault() action="{{route("proyectos.update", $proyecto->id)}}" id="formulario{{$proyecto->id}}" method="POST" class="mt-4">
            ...
            <div>
                <x-input-label for="fecha_de_comienzo" :value="__('Fecha de comienzo')" />
            ...
            <div class="p-2 flex space-x-2">
                ...
                <a class="mt-6 bg-gray-300 text-black px-6 py-2 rounded-lg hover:bg-gray-400" href="{{ route('proyectos.index') }}">
                    {{ __('Cancelar') }}
                </a>
            </div>
        </form>
    ...
    <script>
        function confirmUpdate(id) {
            swal({
                title: "{{ __('¿Deseas actualizar este proyecto?') }}",
